...
export default function VariableSuggestions({
  ...
  // Get variables based on category
  const utilityVariables = getUtilityVariables();
  const marketingVariables = getMarketingVariables();
  const currentVariables = category === 'UTILITY' ? utilityVariables : marketingVariables;
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      ...
      {/* Variables based on category */}
      {category === 'UTILITY' && (
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          <h4 className="font-medium text-gray-900 mb-3 flex items-center gap-2">
            <span className="w-2 h-2 bg-blue-500 rounded-full"></span>
            Utility Variables (Order & Customer-related)
          </h4>
          ...
          <p className="text-xs text-gray-500 mt-2">
            Use these variables for order confirmations, shipping updates, customer service messages, and personalized communications.
          </p>
        </div>
      )}
      ...
      <div className="bg-[#F5F2E8] border border-[#D4AF37] rounded-lg p-4">
        ...
        <div className="text-sm text-[#8B7A1A] space-y-1">
          ...
          <p>• Utility variables: Order confirmations, shipping updates, customer service</p>
          <p>• Marketing variables: Promotional messages and campaigns</p>
          <p>• Variables change based on the selected category</p>
        </div>
      </div>
    </div>
  );
