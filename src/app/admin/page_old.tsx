...
//   const [activeViewerTab, setActiveViewerTab] = useState<Quote | null>(null);
...
//     setQuoteForm({
//       title: '', // Quote interface doesn't have title
//       description: '', // Quote interface doesn't have description
//       items: quote.items || [{ productId: '', quantity: 1 }],
//       terms: '', // Quote interface doesn't have terms
...
//                 // Add default values for missing shipping fields (for backward compatibility)
...
//           try {
//             // Handle createdAt - use current date as fallback if missing
//             if (quote.createdAt instanceof Date) {
//               createdAt = quote.createdAt;
//             } else if (quote.createdAt) {
//               createdAt = new Date(quote.createdAt);
//             } else {
//               // If createdAt is missing, use updatedAt or current date
//               createdAt = quote.updatedAt instanceof Date ? quote.updatedAt : 
//                          quote.updatedAt ? new Date(quote.updatedAt) : new Date();
//             }
            
//             // Handle updatedAt
//             if (quote.updatedAt instanceof Date) {
//               updatedAt = quote.updatedAt;
//             } else if (quote.updatedAt) {
...
//             createdAt,
//             updatedAt
//             // Note: validUntil is a string, not a Date
//           };
...
//       // Check for duplicates before updating (excluding the current lead being edited)
//       if (leadForm.email || leadForm.phone) {
//         const duplicateCheck = await AuthService.checkDuplicateLead(leadForm.email, leadForm.phone);
//         if (duplicateCheck.isDuplicate && duplicateCheck.existingLead?.id !== editingLead.id) {
//           setDuplicateWarning(duplicateCheck);
//           setSubmitError(`Cannot update lead: ${duplicateCheck.reason}`);
...
//   // Helper to find potential duplicate leads
//   function findPotentialDuplicates(leadsList: Lead[]): Array<{ type: 'email' | 'phone'; value: string; leads: Lead[] }> {
//     const duplicates: Array<{ type: 'email' | 'phone'; value: string; leads: Lead[] }> = [];
...
//       if (leads.length > 1) {
//         duplicates.push({ type: 'email', value: email, leads });
//       }
//     });
...
//       if (leads.length > 1) {
//         duplicates.push({ type: 'phone', value: phone, leads });
//       }
//     });
...
//         customerInterest: lead.interest || ''
//       });
//     } else {
//       // New customer mode
...
//     const selectedDate = new Date(quoteForm.validUntil);
//     const today = new Date();
//     today.setHours(0, 0, 0, 0);
    
//     if (selectedDate < today) {
//       showToast('Valid until date cannot be in the past', 'error');
//       return;
//     }
...
//         customerId: quoteMode === 'existing' ? (selectedLeadForQuote?.id || '') : '',
...
//         shippingCharges: quoteForm.shippingCharges,
...
//                             {/* Duplicate Indicator */}
//                             {(() => {
//                               const potentialDuplicates = findPotentialDuplicates(leads);
//                               const hasEmailDuplicate = potentialDuplicates.some(d => 
//                                 d.type === 'email' && d.value === lead.email?.toLowerCase().trim() && d.leads.length > 1
//                               );
//                               const hasPhoneDuplicate = potentialDuplicates.some(d => 
//                                 d.type === 'phone' && d.value === lead.phone.replace(/\D/g, '') && d.leads.length > 1
//                               );
...
//                     Track quotes, manage statuses, and generate PDFs.
//                   </p>
//                   <button 
//                     onClick={() => setShowCreateQuoteModal(true)}
...
//                       <h4 className="text-sm font-semibold text-amber-800">Duplicate Detection</h4>
//                     </div>
//                     <button
//                       onClick={async () => {
//                         try {
...
//                             showToast(`Found ${potentialDuplicates.length} potential duplicate(s)`, 'info');
//                           } else {
...
//                     Our system automatically prevents duplicate leads based on email and phone numbers. 
//                     Use this tool to identify any existing duplicates in your database.
//                   </p>
//                   <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
//                     <div className="bg-white p-2 rounded border border-amber-200">
//                       <span className="font-medium text-amber-800">Email-based:</span>
//                       <span className="ml-1 text-amber-700">
//                         {leads.filter(lead => lead.email).length} leads with emails
//                       </span>
//                     </div>
//                     <div className="bg-white p-2 rounded border border-amber-200">
//                       <span className="font-medium text-amber-800">Phone-based:</span>
//                       <span className="ml-1 text-amber-700">
//                         {leads.filter(lead => lead.phone).length} leads with phones
//                       </span>
//                     </div>
//                     <div className="bg-white p-2 rounded border border-amber-200">
//                       <span className="font-medium text-amber-800">Unique:</span>
//                       <span className="ml-1 text-amber-700">
//                         {new Set(leads.map(lead => lead.email || lead.phone)).size} unique identifiers
//                       </span>
//                     </div>
//                   </div>
//                 </div>
...
//                           <p className="text-xs font-medium text-amber-800 mb-2">Existing Lead Details:</p>
//                           <div className="text-xs text-amber-700 space-y-1">
//                             <p><span className="font-medium">Name:</span> {duplicateWarning.existingLead.name}</p>
//                             <p><span className="font-medium">Phone:</span> {duplicateWarning.existingLead.phone}</p>
//                             {duplicateWarning.existingLead.email && (
//                               <p><span className="font-medium">Email:</span> {duplicateWarning.existingLead.email}</p>
//                             )}
//                             <p><span className="font-medium">Status:</span> {duplicateWarning.existingLead.status}</p>
//                             <p><span className="font-medium">Created:</span> {formatDate(duplicateWarning.existingLead.createdAt)}</p>
//                           </div>
//                         </div>
//                       )}
...
//                             type="button"
//                             onClick={() => {
//                               // Pre-fill form with existing lead data for editing
//                               const existingLead = duplicateWarning.existingLead!;
//                               setLeadForm({
...
//             <form onSubmit={handleUpdateLead} className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5 sm:space-y-6">
...
//                       onChange={handleInputChange}
//                       required
//                       className={`w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:border-transparent text-base touch-manipulation ${
//                         duplicateWarning?.isDuplicate && duplicateWarning.reason?.includes('phone')
//                           ? 'border-red-300 focus:ring-red-300 text-red-600 placeholder-red-400'
//                           : 'border-[#D4AF37] focus:ring-[#D4AF37] text-[#5E4E06] placeholder-[#8B7A1A]'
//                       }`}
...
//                   </div>
//                   {duplicateWarning?.isDuplicate && duplicateWarning.reason?.includes('phone') && (
//                     <p className="text-xs text-red-600">Phone number already exists in another lead</p>
//                   )}
//                 </div>
...
//                   </div>
//                   {duplicateWarning?.isDuplicate && duplicateWarning.reason?.includes('email') && (
//                     <p className="text-xs text-red-600">Email already exists in another lead</p>
//                   )}
//                 </div>
//               </div>
...
//                 </div>
//                 <div>
//                   <h3 className="text-lg sm:text-xl font-bold text-[#5E4E06]">Edit Quote</h3>
//                   <p className="text-xs sm:text-sm text-[#8B7A1A]">Update quote information</p>
//                 </div>
//               </div>
//               <button 
//                 onClick={() => setShowEditQuoteModal(false)}
//                 className="p-2 hover:bg-[#E6DCC0] rounded-lg transition-colors duration-200"
//               >
//                 <X className="w-5 h-5 text-[#8B7A1A]" />
//               </button>
//             </div>
...
//               <div className="bg-[#F5F2E8] rounded-xl p-4 sm:p-6 border border-[#D4AF37]">
//                 <h4 className="text-base sm:text-lg font-semibold text-[#5E4E06] mb-4">Customer Information</h4>
//                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
//                   <div>
...
//                       type="text"
//                       name="customerName"
//                       value={quoteForm.customerName || editingQuote.customerName}
//                       onChange={handleInputChange}
//                       className="w-full px-4 py-3 border border-[#D4AF37] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#D4AF37] focus:border-transparent text-[#5E4E06]"
...
//                   </div>
//                   <div>
//                     <label className="block text-sm font-medium text-[#8B7A1A] mb-2">Customer Email</label>
//                     <input
//                       type="email"
//                       name="customerEmail"
//                       value={quoteForm.customerEmail || editingQuote.customerEmail}
//                       onChange={handleInputChange}
//                       className="w-full px-4 py-3 border border-[#D4AF37] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#D4AF37] focus:border-transparent text-[#5E4E06]"
...
//                   </div>
//                   <div>
//                     <label className="block text-sm font-medium text-[#8B7A1A] mb-2">Customer Phone</label>
//                     <input
//                       type="tel"
//                       name="customerPhone"
//                       value={quoteForm.customerPhone || editingQuote.customerPhone}
//                       onChange={handleInputChange}
//                       className="w-full px-4 py-3 border border-[#D4AF37] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#D4AF37] focus:border-transparent text-[#5E4E06]"
...
//                   </div>
//                   <div>
//                     <label className="block text-sm font-medium text-[#8B7A1A] mb-2">Product Interest</label>
//                     <select
//                       name="customerInterest"
//                       value={quoteForm.customerInterest || editingQuote.customerInterest}
//                       onChange={handleInputChange}
//                       className="w-full px-4 py-3 border border-[#D4AF37] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#D4AF37] focus:border-transparent text-[#5E4E06]"
//                     >
...
//                       type="date"
//                       name="validUntil"
//                       value={quoteForm.validUntil || editingQuote.validUntil}
//                       onChange={handleInputChange}
//                       className="w-full px-4 py-3 border border-[#D4AF37] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#D4AF37] focus:border-transparent text-[#5E4E06]"
//                     />
//                   </div>
//                   <div>
//                     <label className="block text-sm font-medium text-[#8B7A1A] mb-2">Status</label>
//                     <select
//                       name="status"
//                       value={quoteForm.status || editingQuote.status}
//                       onChange={handleInputChange}
//                       className="w-full px-4 py-3 border border-[#D4AF37] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#D4AF37] focus:border-transparent text-[#5E4E06]"
//                     >
...
//                   </div>
//                   <div>
//                     <label className="block text-sm font-medium text-[#8B7A1A] mb-2">Discount</label>
//                     <input
//                       type="number"
//                       name="discount"
//                       value={quoteForm.discount || editingQuote.discount}
//                       onChange={handleInputChange}
//                       className="w-full px-4 py-3 border border-[#D4AF37] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#D4AF37] focus:border-transparent text-[#5E4E06]"
//                       placeholder="0"
//                     />
//                   </div>
//                   <div>
//                     <label className="block text-sm font-medium text-[#8B7A1A] mb-2">Discount Type</label>
//                     <select
//                       name="discountType"
//                       value={quoteForm.discountType || editingQuote.discountType}
//                       onChange={handleInputChange}
//                       className="w-full px-4 py-3 border border-[#D4AF37] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#D4AF37] focus:border-transparent text-[#5E4E06]"
//                     >
...
//                   </div>
//                   <div>
//                     <label className="block text-sm font-medium text-[#8B7A1A] mb-2">Shipping Charges</label>
//                     <input
//                       type="number"
//                       name="shippingCharges"
//                       value={quoteForm.shippingCharges || editingQuote.shippingCharges}
//                       onChange={handleInputChange}
//                       className="w-full px-4 py-3 border border-[#D4AF37] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#D4AF37] focus:border-transparent text-[#5E4E06]"
//                       placeholder="0"
//                     />
...
//                       checked={quoteForm.includeShipping || editingQuote.includeShipping}
//                       onChange={(e) => setQuoteForm(prev => ({ ...prev, includeShipping: e.target.checked }))}
//                       className="w-4 h-4 text-[#D4AF37] border-[#D4AF37] rounded focus:ring-[#D4AF37]"
//                     />
//                     <label className="text-sm text-[#8B7A1A]">Include Shipping</label>
//                   </div>
//                 </div>
//               </div>
...
//               {/* Payment Link */}
//               <div className="bg-[#F5F2E8] rounded-xl p-4 sm:p-6 border border-[#D4AF37]">
//                 <h4 className="text-base sm:text-lg font-semibold text-[#5E4E06] mb-4">Payment</h4>
//                 <div>
//                   <label className="block text-sm font-medium text-[#8B7A1A] mb-2">Payment Link</label>
//                   <input
//                     type="url"
//                     name="paymentLink"
//                     value={quoteForm.paymentLink || editingQuote.paymentLink}
//                     onChange={handleInputChange}
//                     className="w-full px-4 py-3 border border-[#D4AF37] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#D4AF37] focus:border-transparent text-[#5E4E06]"
...
//                 <button
//                   onClick={() => setShowEditQuoteModal(false)}
//                   className="px-6 py-3 bg-[#F5F2E8] text-[#8B7A1A] rounded-xl hover:bg-[#E6DCC0] transition-colors"
//                 >
//                   Cancel
//                 </button>
//                 <button
//                   onClick={handleEditQuote}
//                   className="px-8 py-3 bg-gradient-to-r from-[#D4AF37] to-[#8B7A1A] text-white rounded-xl hover:scale-105 transition-all duration-200 shadow-lg font-medium"
//                 >
//                   Update Quote
//                 </button>
//               </div>
...
//                         <p className="font-medium text-[#5E4E06]">{activeViewerTab.customerName}</p>
//                       </div>
//                       <div>
//                         <span className="text-sm text-[#8B7A1A]">Email:</span>
//                         <p className="font-medium text-[#5E4E06]">{activeViewerTab.customerEmail}</p>
//                       </div>
//                       <div>
//                         <span className="text-sm text-[#8B7A1A]">Phone:</span>
//                         <p className="font-medium text-[#5E4E06]">{activeViewerTab.customerPhone}</p>
//                       </div>
//                       <div>
//                         <span className="text-sm text-[#8B7A1A]">Interest:</span>
//                         <p className="font-medium text-[#5E4E06]">{activeViewerTab.customerInterest}</p>
//                       </div>
//                     </div>
//                   </div>
...
//                     <div className="space-y-3">
//                       {activeViewerTab.items.map((item, index) => {
//                         const product = products.find(p => p.id === item.productId);
//                         return (
...
//                         <span className="text-[#8B7A1A]">Subtotal:</span>
//                         <span className="font-medium text-[#5E4E06]">₹{activeViewerTab.subtotal.toLocaleString()}</span>
//                       </div>
//                       <div className="flex justify-between">
//                         <span className="text-[#8B7A1A]">Discount ({activeViewerTab.discountType === 'percentage' ? `${activeViewerTab.discount}%` : `₹${activeViewerTab.discount}`}):</span>
//                         <span className="font-medium text-[#5E4E06]">₹{activeViewerTab.discountType === 'percentage' ? (activeViewerTab.subtotal * activeViewerTab.discount / 100) : activeViewerTab.discount}</span>
//                       </div>
//                       {activeViewerTab.includeShipping ? (
//                         <div className="flex justify-between">
//                           <span className="text-[#8B7A1A]">Shipping Charges:</span>
//                           <span className="font-medium text-[#5E4E06]">₹{activeViewerTab.shippingCharges}</span>
//                         </div>
//                       ) : (
//                         <div className="flex justify-between">
//                           <span className="text-[#8B7A1A]">Shipping Charges:</span>
...
//                           <span className="text-[#5E4E06]">Total:</span>
//                           <span className="text-[#D4AF37]">₹{activeViewerTab.total.toLocaleString()}</span>
//                         </div>
//                       </div>
//                     </div>
//                   </div>
...
//                         <p className="font-medium text-[#5E4E06]">{new Date(activeViewerTab.validUntil).toLocaleDateString()}</p>
//                       </div>
...
//                       <button
//                         onClick={() => handlePreviewPDF(activeViewerTab)}
//                         className="px-4 py-2 bg-[#D4AF37] text-white rounded-lg hover:bg-[#8B7A1A] transition-colors"
//                       >
//                         📄 Preview PDF
//                       </button>
//                       <button
//                         onClick={() => handleDownloadPDF(activeViewerTab)}
//                         className="px-4 py-2 bg-[#8B7A1A] text-white rounded-lg hover:bg-[#5E4E06] transition-colors"
//                       >
//                         💾 Download PDF
//                       </button>
//                     </div>
